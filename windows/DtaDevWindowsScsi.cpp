...
DtaDevWindowsScsi *
DtaDevWindowsScsi::getDtaDevWindowsScsi(const char * devref, DTA_DEVICE_INFO & device_info) {

  OSDEVICEHANDLE osDeviceHandle = fdopen(devref);
  if (osDeviceHandle < 0)
    return NULL;
  ...
  return new DtaDevWindowsScsi(osDeviceHandle);
}



bool DtaDevWindowsScsi::identifyUsingSCSIInquiry(OSDEVICEHANDLE osDeviceHandle,
                                                 InterfaceDeviceID & interfaceDeviceIdentification,
                                                 DTA_DEVICE_INFO & device_info) {
  if (!deviceIsStandardSCSI(osDeviceHandle, interfaceDeviceIdentification, device_info)) {
...
#undef ALLOW_INQUIRY_PAGE_00_FAILURES
  ...
  if (deviceSupportsPage80) {
    if (deviceIsPage80SCSI(osDeviceHandle, interfaceDeviceIdentification, device_info)) {
  ...
  if (deviceIsPage83SCSI(device_info)) {
  ...
  if (deviceSupportsPage89) {
    if (deviceIsPage89SCSI(device_info)) {
...
bool DtaDevWindowsScsi::deviceIsStandardSCSI(OSDEVICEHANDLE osDeviceHandle, InterfaceDeviceID & interfaceDeviceIdentification, DTA_DEVICE_INFO &device_info)
{
...
int DtaDevWindowsScsi::inquiryStandardDataAll_SCSI(OSDEVICEHANDLE osDeviceHandle, void * inquiryResponse, unsigned int & dataSize )
{
...
int DtaDevWindowsScsi::__inquiry(OSDEVICEHANDLE osDeviceHandle, uint8_t evpd, uint8_t page_code, void * inquiryResponse, unsigned int & dataSize)
{
...
dictionary *
DtaDevWindowsScsi::parseInquiryStandardDataAllResponse(const unsigned char * response,
                                                     InterfaceDeviceID & interfaceDeviceIdentification,
                                                     DTA_DEVICE_INFO & device_info)
{
  const CScsiCmdInquiry_StandardData *resp = reinterpret_cast <const CScsiCmdInquiry_StandardData *>(response);

  memcpy(interfaceDeviceIdentification, (InterfaceDeviceID *)(&(resp->m_T10VendorId)), sizeof(InterfaceDeviceID));

  safecopy(device_info.vendorID, sizeof(device_info.vendorID), resp->m_T10VendorId, sizeof(resp->m_T10VendorId));
...
bool DtaDevWindowsScsi::deviceIsPage00SCSI(OSDEVICEHANDLE osDeviceHandle,
...
int DtaDevWindowsScsi::inquiryPage00_SCSI(OSDEVICEHANDLE osDeviceHandle, void* buffer, unsigned int& dataSize)
{
    return __inquiry__EVPD(osDeviceHandle, kINQUIRY_Page00_PageCode, buffer, dataSize);
}

dictionary* DtaDevWindowsScsi::parseInquiryPage00Response(const unsigned char* response,
...
bool DtaDevWindowsScsi::deviceIsPage80SCSI(OSDEVICEHANDLE osDeviceHandle, const InterfaceDeviceID& interfaceDeviceIdentification,
    DTA_DEVICE_INFO& device_info)
{
    // Test whether device is a SCSI drive by attempting
    // SCSI Inquiry command
...
int DtaDevWindowsScsi::inquiryPage80_SCSI(OSDEVICEHANDLE osDeviceHandle, void* buffer, unsigned int& dataSize)
{
    return __inquiry__EVPD(osDeviceHandle, kINQUIRY_Page80_PageCode, buffer, dataSize);
}


int DtaDevWindowsScsi::__inquiry__EVPD(OSDEVICEHANDLE osDeviceHandle, uint8_t page_code,
...
//static char* _strrev(char* cString) {
...
dictionary* DtaDevWindowsScsi::parseInquiryPage80Response(const InterfaceDeviceID& interfaceDeviceIdentification,
    ...
    DTA_DEVICE_INFO& device_info)
{
    ...
    memcpy(device_info.passwordSalt, serialNumber, sizeof(device_info.passwordSalt));  // save value before polishing
    if (deviceNeedsSpecialAction(interfaceDeviceIdentification,
        ...
        _strrev((char*)serialNumber);
    }
    LOG(D4) << "Inquiry Page80 serial number is " << serialNumber;
    memcpy(device_info.serialNum, serialNumber, sizeof(device_info.serialNum));
...
uint8_t DtaDevWindowsScsi::sendCmd(ATACOMMAND cmd, uint8_t protocol, uint16_t comID,
                                 void * buffer, uint32_t bufferlen)
{
  LOG(D4) << "Entering DtaDevWindowsScsi::sendCmd";
    ...
    default:
      {
        LOG(D4) << "Unknown cmd=0x" << std::hex << cmd
                << " -- returning 0xff from DtaDevWindowsScsi::sendCmd";
        return 0xff;
      }
    }
  ...
  // success
  LOG(D4) << "Returning 0x00 from DtaDevWindowsScsi::sendCmd";
  return 0x00;
}

bool  DtaDevWindowsScsi::identify(DTA_DEVICE_INFO& device_info)
{
  InterfaceDeviceID interfaceDeviceIdentification;
  return identifyUsingSCSIInquiry(osDeviceHandle, interfaceDeviceIdentification, device_info);
}


/** Send a SCSI command using an ioctl to the device. */

int DtaDevWindowsScsi::PerformSCSICommand(OSDEVICEHANDLE osDeviceHandle,
                                        int dxfer_direction,
 ...
 */
 /** Device specific implementation of disk access functions. */
  ...
  if( pmasked_status != NULL) *pmasked_status = scsi->sd.ScsiStatus;
