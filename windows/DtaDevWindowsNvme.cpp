 ...
 *  Windows specific implementation using the NVMe interface
 */

DtaDevWindowsNvme * DtaDevWindowsNvme::getDtaDevWindowsNvme(const char * devref,
  ...
  DtaDevWindowsNvme * drive = new DtaDevWindowsNvme((int)fd_);
...
uint8_t DtaDevWindowsNvme::sendCmd(ATACOMMAND cmd, uint8_t protocol, uint16_t comID,
                                 void * buffer, uint32_t bufferlen)
{
  LOG(D1) << "Entering DtaDevWindowsNvme::sendCmd";
  ...
  int err = ioctl(osDeviceHandle, NVME_IOCTL_ADMIN_CMD, &nvme_cmd);
...
bool DtaDevWindowsNvme::identifyUsingNvmeIdentify(OSDEVICEHANDLE osDeviceHandle,
                                                InterfaceDeviceID &,  // Currently unused
                                                DTA_DEVICE_INFO & disk_info)
{
  LOG(D4) << "Entering DtaDevWindowsNvme::identifyUsingNvmeIdentify";
  ...
  int err = ioctl(osDeviceHandle, NVME_IOCTL_ADMIN_CMD, &cmd);
